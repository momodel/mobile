...
class ChatBot extends Component {
  ...
  // 点击底部custom option后
  triggerCustomOption(customOptionsStep, data) {
    // 更改 renderedSteps, previousSteps, currentStep, previousStep
    const {
      renderedSteps,
      previousSteps,
      steps,
    ...
    // 将 currentStep 推到 renderedSteps, previousSteps,

    if (isEnd) {
      this.handleEnd()
    } else if (data) {
      const option = customOptionsStep.options.filter(
        o => o.value === data.value
      )[0]
      const trigger = this.getTriggeredStep(option.trigger, currentStep.value)
      let nextStep = Object.assign({}, option, defaultUserSettings, {
        user: true,
        message: option.label,
        trigger,
      })
      ...
      // const secondTrigger = this.getTriggeredStep(
      //   currentStep.trigger,
      //   currentStep.value
      // )
  ...
  renderStep(step, index) {
    ...
    if (component && !asMessage) {
      const customOptionsStep = this.formatStep(optionStep)
      return (
        <CustomStep
          key={index}
          delay={customDelay}
  ...
  render() {
    const {
      ...
      inputValue,
      renderedSteps,
      // previousSteps, steps, currentStep, previousStep
    } = this.state
    const {
      botBubbleColor,
      botFontColor,
    ...
    /***************************************************** start *****************************************************/
    const customOptionStep = this.formatStep(optionStep)
    /***************************************************** end *****************************************************/
